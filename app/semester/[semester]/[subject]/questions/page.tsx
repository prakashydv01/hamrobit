      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex justify-between items-start flex-wrap gap-4">
            ...
            {!loading && data.length > 0 && (
              <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg border border-blue-100">
                ...
                <span className="text-sm text-blue-700">{data.length} Question Papers Available</span>
              </div>
            )}
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="mt-4 text-gray-500">Loading model questions...</p>
          </div>
        ) : data.length === 0 ? (
          <div className="text-center py-20">
            <HelpCircle className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <p className="text-red-500 text-lg">No questions found.</p>
            <p className="text-gray-400 mt-2">Model questions will be available soon</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {data.map((q, i) => {
              const pdf = getAsset(q.fields?.pdf);
              const image = getAsset(q.fields?.image);
                  ...
                  <div className="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {q.fields.title && (
                          <p className="text-sm text-gray-500 mt-0.5">{q.fields.title}</p>
                        )}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-100">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm text-yellow-700">
                            No files attached to this question paper.
                          </p>
                        </div>
                      </div>
                    )}
